...
Now, we will go through all the steps of the resampling procedure so that every user of the CzechVeg data can prepare an alternative resampled and stratified dataset according to their own criteria. We will start with the CzechVeg-Open dataset. After removing plots without geographical coordinates and those containing only non-vascular plants, the dataset contains 116,156 vegetation plots as shown below. Yellow areas are those with the highest plot density.
...
The first object required by the `resample` function (`coord`) must be a `data.table` containing at least three columns including plot IDs and the longitude and latitude (or X and Y) coordinates of vegetation plots. Optionally, this `data.table` may include additional columns with environmental strata and a variable used to decide which vegetation plot will be removed from each pair of plots that are closer and more compositionally similar than specified thresholds. In our case, we will use environmental strata defined based on principal component analysis (PCA) of XXX environmental factors (details can be found here). However, in the full CzechVeg-Open dataset we provide also alternative environmental stratifications of the country that are based on published literature and can be used as an alternative to our PCA-based strata. Finally, we will use also the year of sampling to prefer younger plots in the resampling procedure. Only this variable may contain `NA` values and the corresponding plots are removed first. So, let’s read the the header data and create `data.table`:
...
#| label: Renaming
#| eval: false
colnames(coord_sel)[1] <- "PlotObservationID"
```
...
#| eval: false
spe <- read.csv("species_data.csv", header=T)
head(spe)
```

Columns must have specific names: `PlotObservationID`, `Taxon_name` and `cover`.

```{r}
#| label: Renaming columns
#| eval: false
colnames(spe_sel) <- c("PlotObservationID", "Taxon_name", "cover")
```

Now, we are ready to run the stratified resampling of the CzechVeg-Open dataset. Let’t load the function:
...
We will use moderate criteria to remove only the closest and most compositionally similar plots in each stratum:

-   `longlat = FALSE` Because we use coordinates in the WGS 84 / UTM zone 33N coordinate system, this argument must be set to `FALSE`.

-   `dist.threshold = 1000` With this setting, the function will consider plots closer than 1000 m as neighbors. Within this neighborhood, it will search for conflicting pairs that exceed the similarity threshold. Note that the value set depends on the coordinate system used. Please see the detailed description [here](https://github.com/jdivisek/FilteringReleves).

-   `sim.threshold = 0.8` Pairs of neighboring plots with similarity exceeding this value will be resampled.
...
-   `strata = "env_class"` This argument specifies the column in `coord` that contains environmental stratification. Plots will be resampled separately within each stratum.
